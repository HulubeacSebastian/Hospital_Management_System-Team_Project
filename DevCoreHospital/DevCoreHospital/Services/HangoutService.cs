...
namespace DevCoreHospital.Services
{
    public class HangoutService
    {
        ...
        public void CreateHangout(string title, string description, DateTime date, int maxParticipants, IStaff creator)
        {
            ...
            // Validate that the creator has no appointments or medical evaluations scheduled for that day.
            if (hangoutRepository.HasConflictsOnDate(creator.StaffID, date))
                throw new InvalidOperationException("You cannot create a hangout on a day where you have scheduled appointments.");
        ...
        public void JoinHangout(int hangoutId, IStaff staff)
        {
            ...
            // Validate that the staff member has no appointments or medical evaluations scheduled for that day.
            if (hangoutRepository.HasConflictsOnDate(staff.StaffID, hangout.date))
                throw new InvalidOperationException("You cannot join a hangout on a day where you have scheduled appointments.");
